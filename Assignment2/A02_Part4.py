...
def my_model(ssc, monitoring_dir, window_duration, sliding_duration, time_step_interval):
  ...
  solution = inputStream.filter(lambda row : row.split(";")[0]=="0" and row.split(";")[5]=="0").map(lambda x: (x.split(';')[1],1) ).reduceByKey( lambda x,y:x+y )
  solution.persist(pyspark.StorageLevel.MEMORY_AND_DISK)
  solution.pprint()
